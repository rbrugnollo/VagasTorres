...
function setLastAssignmentPDF(dataUrl) {
  localStorage.setItem(LAST_ASSIGNMENT_KEY, JSON.stringify({ date: new Date().toISOString(), pdf: dataUrl }));
}
...
function renderAssignmentPage() {
  ...
  app.innerHTML = `
    <div class="mb-4">
      ...
      <span class="ml-4 text-gray-600">${lastDate ? `Último sorteio: ${lastDate.toLocaleDateString()}` : 'Nunca sorteado'}</span>
    </div>
...
function assignSpots(apartments, spots, preassigned) {
  ...
  // Generate PDF
  generatePDF(apartments, spots, preassigned, assignments);
}

function generatePDF(apartments, spots, preassigned, assignments) {
  ...
  y += 10;
  doc.text('Apartamentos:', 10, y);
  y += 8;
  apartments.forEach(u => { doc.text(`- ${u}`, 12, y); y += 6; });
  y += 4;
  doc.text('Vagas de Estacionamento:', 10, y);
  y += 8;
  spots.forEach(s => { doc.text(`- ${s}`, 12, y); y += 6; });
  y += 4;
  doc.text('Pré-Atribuídos:', 10, y);
  y += 8;
  preassigned.forEach(p => { doc.text(`- ${p.unit} → ${p.spot}`, 12, y); y += 6; });
  y += 4;
  doc.text('Sorteio Final:', 10, y);
  y += 8;
  assignments.forEach(a => { doc.text(`- ${a.unit} → ${a.spot}${preassigned.some(p => p.unit === a.unit) ? ' (Pré-Atribuído)' : ''}`, 12, y); y += 6; });
  // Save PDF to localStorage
  const pdfData = doc.output('dataurlstring');
  setLastAssignmentPDF(pdfData);
  document.getElementById('pdfSection').innerHTML = `<a href="${pdfData}" download="assignment.pdf" class="bg-green-500 text-white px-4 py-2 rounded">Download do Último sorteio (PDF)</a>`;
}
